<script lang="ts">
  ...
  const encryptKey = async () => {
    ...
    try {
      const fetcher = useFetch<RegisterRequestData, RegisterResponseData>('POST', '/api/v2/auth/register');
      await fetcher.fetch({
        uuid: uuid(),
...
<!-- TODO: translate -->
<Modal width={25} bind:opened header="Enable encryption">
  {#if step === 1}
  ...
  {:else if step === 2}
    <form on:submit|preventDefault={encryptKey} class="flex-col gap-1">
      ...
      <Input label="Password" bind:value={password} type="password" required minlength={6} />
      <Input label="Repeat password" bind:value={repeatPassword} type="password" required minlength={6} />
