...
<ListGroup title={$translate('settings.profile')}>
  <ListSelectItem title={$translate('auth.name')} value={data.user?.name ?? ''} on:click={openChangeNameModal} />
...
<div class="mt-1 flex-col items-center gap-0.5 ">
  ...
  <div class="build-info ">
    <span>{$translate('settings.version', { values: { version: data.version } })}</span>
    <span>•</span>
...
<ChangeNameModal bind:opened={$changeNameModalOpened} userId={data.userId} value={data.user?.name} />
<ChangeLoginModal bind:opened={$changeLoginModalOpened} userId={data.userId} value={data.user?.login} />
<ChangePasswordModal bind:opened={$changePasswordModalOpened} userId={data.userId} />
<SelectGroupModal
  bind:opened={$selectGroupModalOpened}
  userId={data.userId}
