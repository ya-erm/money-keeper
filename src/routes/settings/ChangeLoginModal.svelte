<script lang="ts">
  ...
  const handleSubmit = async () => {
    ...
    if (response.ok) {
      ...
      showErrorToast($translate('settings.profile.change_login_failure'));
    }
  };
</script>

<Modal header={$translate('settings.profile.change_login')} bind:opened>
  <form class="flex-col gap-1" on:submit|preventDefault={handleSubmit}>
    <Input label={$translate('auth.login')} name="login" testId="ChangeLoginInput" bind:value={_value} required />
